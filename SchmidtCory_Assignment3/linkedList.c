...
static void init(struct LinkedList* list) {
    ...
    back = (struct Link *)malloc(sizeof(struct Link));
...
void linkedListPrint(struct LinkedList* list)
{
    ...
    struct Link *printValues = list->frontSentinel->next;
    
    assert(printValues != 0);
    
    while (printValues != list->backSentinel) {
        printf(" %d ", printValues->value);
